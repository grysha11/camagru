...
type UserResponse struct {
	...
	CreatedAt       time.Time           `json:"created_at"`
	Email           openapi_types.Email `json:"email"`
	HasGithubLogin  bool                `json:"has_github_login"`
	Id              openapi_types.UUID  `json:"id"`
...
type ServerInterface interface {
	...
	// Update the current user's username, email, or comment-notification preference
	// (PATCH /api/me)
	...
	// Start linking the authenticated user's account to GitHub OAuth2
	// (GET /api/oauth/github/link)
	GitHubOAuthLink(w http.ResponseWriter, r *http.Request)
	...
	// Email the current authenticated user a password reset link
...
// GitHubOAuthLink operation middleware
func (siw *ServerInterfaceWrapper) GitHubOAuthLink(w http.ResponseWriter, r *http.Request) {

	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.GitHubOAuthLink(w, r)
	}))
...
type GitHubOAuthLinkRequestObject struct {
}

type GitHubOAuthLinkResponseObject interface {
	VisitGitHubOAuthLinkResponse(w http.ResponseWriter) error
}

type GitHubOAuthLink302ResponseHeaders struct {
	Location  *string
	SetCookie *string
}

type GitHubOAuthLink302Response struct {
	Headers GitHubOAuthLink302ResponseHeaders
}

func (response GitHubOAuthLink302Response) VisitGitHubOAuthLinkResponse(w http.ResponseWriter) error {
	if response.Headers.Location != nil {
		w.Header().Set("Location", fmt.Sprint(*response.Headers.Location))
	}
	if response.Headers.SetCookie != nil {
...
type GitHubOAuthLink401JSONResponse ErrorResponse

func (response GitHubOAuthLink401JSONResponse) VisitGitHubOAuthLinkResponse(w http.ResponseWriter) error {
...
type GitHubOAuthLink500JSONResponse ErrorResponse

func (response GitHubOAuthLink500JSONResponse) VisitGitHubOAuthLinkResponse(w http.ResponseWriter) error {
...
type ListMyPosts200JSONResponse []PostSummary
...
type StrictServerInterface interface {
	...
	// Start linking the authenticated user's account to GitHub OAuth2
	// (GET /api/oauth/github/link)
	GitHubOAuthLink(ctx context.Context, request GitHubOAuthLinkRequestObject) (GitHubOAuthLinkResponseObject, error)
	// Start GitHub OAuth2 login
	// (GET /api/oauth/github/login)
...
// GitHubOAuthLink operation middleware
func (sh *strictHandler) GitHubOAuthLink(w http.ResponseWriter, r *http.Request) {
	var request GitHubOAuthLinkRequestObject

	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.GitHubOAuthLink(ctx, request.(GitHubOAuthLinkRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "GitHubOAuthLink")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(GitHubOAuthLinkResponseObject); ok {
		if err := validResponse.VisitGitHubOAuthLinkResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
